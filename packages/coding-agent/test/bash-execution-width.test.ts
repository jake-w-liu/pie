...
describe("BashExecutionComponent width handling (#2569)", () => {
	...
	it("retains only a bounded display tail while output streams", () => {
		const { stub } = createTuiStub(120);
		const component = new BashExecutionComponent("generate", stub);
		const chunk = Array.from({ length: 100 }, (_, index) => `line-${index} ${"x".repeat(100)}`).join("\n");

		for (let index = 0; index < 500; index++) component.appendOutput(`${chunk}\n`);

		const retained = component.getOutput();
		expect(Buffer.byteLength(retained, "utf8")).toBeLessThanOrEqual(DEFAULT_MAX_BYTES + 1);
		...
		expect(component.render(120).join("\n")).toContain("earlier lines omitted from the UI buffer");
	});
