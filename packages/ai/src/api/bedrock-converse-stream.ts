...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// Stream exceptions have no HTTP metadata; retain the request id for the error boundary.
		...
		try {
			...
			const configuredRegion = arnRegion || getConfiguredBedrockRegion(options);
			...
			// Only pin standard AWS Bedrock runtime endpoints when no region or effective AWS_PROFILE is configured.
	...
	const middleware: DeserializeMiddleware<object, MetadataBearer> = (next) => async (args) => {
		...
		if (providerResponse) {
			...
			try {
				...
				if (isSmithyHttpResponse(result.response)) await cancelResponseBody(result.response);
				throw error;
			}
		}
		return result;
