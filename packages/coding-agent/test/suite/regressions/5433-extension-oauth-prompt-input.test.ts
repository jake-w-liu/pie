...
describe("LoginDialogComponent OAuth prompts", () => {
	...
	test("never renders secret prompt values before or after submission", async () => {
		const dialog = createDialog();
		const prompt = dialog.showPrompt("Enter secret:", undefined, true);
		dialog.handleInput("secret-value");
		dialog.handleInput("\x17"); // Ctrl+W places the secret in the input kill ring.
		dialog.handleInput("\x19"); // Ctrl+Y restores it for submission.

		let output = renderDialog(dialog).join("\n");
		expect(output).not.toContain("secret-value");
		expect(output).toContain("•".repeat("secret-value".length));

		dialog.handleInput("\n");
		await expect(prompt).resolves.toBe("secret-value");
		output = renderDialog(dialog).join("\n");
		expect(output).not.toContain("secret-value");
		expect(output).toContain("[submitted securely]");

		const publicPrompt = dialog.showPrompt("Enter public value:");
		dialog.handleInput("\x19"); // Yank must not recover the previous secret.
		dialog.handleInput("\x1b[45;5u"); // Undo must not recover it either.
		output = renderDialog(dialog).join("\n");
		expect(output).not.toContain("secret-value");
		dialog.handleInput("public-value");
		dialog.handleInput("\n");
		await expect(publicPrompt).resolves.toBe("public-value");
	});

	test("advertises cancellation while waiting on browser authentication", () => {
		const dialog = createDialog();
