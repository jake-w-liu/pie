...
const BUILTIN_THEME_NAMES = ["dark", "light", "pie-nippon-dark", "pie-nippon-light"] as const;
const BUILTIN_THEME_NAME_SET = new Set<string>(BUILTIN_THEME_NAMES);
const BUILTIN_THEMES = new Map<string, ThemeJson>();

export function resolveBuiltinThemeName(name: string, appName = APP_NAME): string {
	if (appName !== "pie") return name;
	if (name === "dark") return "pie-nippon-dark";
	if (name === "light") return "pie-nippon-light";
	return name;
}

function getBuiltinTheme(name: string): ThemeJson | undefined {
	const resolvedName = resolveBuiltinThemeName(name);
	if (!BUILTIN_THEME_NAME_SET.has(resolvedName)) return undefined;
	const cached = BUILTIN_THEMES.get(resolvedName);
	if (cached) return cached;
	const themePath = path.join(getThemesDir(), `${resolvedName}.json`);
	const themeJson = parseThemeJsonContent(themePath, fs.readFileSync(themePath, "utf-8"));
	BUILTIN_THEMES.set(resolvedName, themeJson);
	return themeJson;
}
...
export function getAvailableThemesWithPaths(): ThemeInfo[] {
	...
	for (const name of BUILTIN_THEME_NAMES) {
		addTheme({ name, path: path.join(themesDir, `${resolveBuiltinThemeName(name)}.json`) });
	}
...
function loadThemeJson(name: string): ThemeJson {
	const builtinTheme = getBuiltinTheme(name);
...
function startThemeWatcher(): void {
	...
	if (!currentThemeName || BUILTIN_THEME_NAME_SET.has(currentThemeName)) return;
...
export function isLightTheme(themeName?: string): boolean {
	const resolvedName = resolveBuiltinThemeName(themeName ?? currentThemeName ?? getDefaultTheme());
	return resolvedName === "light" || resolvedName === "pie-nippon-light";
}
