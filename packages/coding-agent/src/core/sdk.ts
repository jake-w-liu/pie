...
import { HEADROOM_RETRIEVE_TOOL_NAME, projectHeadroomContext, transformHeadroomContext } from "./headroom.ts";
...
import { createHeadroomRetrieveToolDefinition } from "./tools/headroom-retrieve.ts";
import {
	createBashTool,
	createCodingTools,
	createEditTool,
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const coreCustomTools: ToolDefinition[] = [
		...(options.customTools ?? []),
		createHeadroomRetrieveToolDefinition() as unknown as ToolDefinition,
	];
	...
	agent = new Agent({
		...
		transformContext: async (messages) => {
			...
			const retrievalAvailable = agent.state.tools.some((tool) => tool.name === HEADROOM_RETRIEVE_TOOL_NAME);
			return transformHeadroomContext(extensionMessages, retrievalAvailable);
		},
	...
	const session = new AgentSession({
		...
		resourceLoader,
		customTools: coreCustomTools,
		modelRuntime,
		projectContextForCompaction: (messages) => {
			const retrievalAvailable = agent.state.tools.some((tool) => tool.name === HEADROOM_RETRIEVE_TOOL_NAME);
			return projectHeadroomContext(messages, retrievalAvailable);
		},
