...
describe("Input component", () => {
	...
	describe("render", () => {
		...
		it("masks secrets without changing the submitted value", () => {
			...
			assert.ok(rendered.includes("•".repeat("secret-value".length)));
			assert.strictEqual(input.getValue(), "secret-value");
		});

		it("secure reset clears undo and kill-ring history", () => {
			const input = new Input();
			input.setMasked(true);
			input.handleInput("secret-value");
			input.handleInput("\x17"); // Ctrl+W stores the secret in the kill ring.
			input.reset();
			input.setMasked(false);

			input.handleInput("\x19"); // Ctrl+Y must not restore the secret.
			input.handleInput("\x1b[45;5u"); // Ctrl+- must not restore an undo snapshot.
