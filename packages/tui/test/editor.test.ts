...
describe("Editor component", () => {
	...
	describe("Autocomplete", () => {
		...
		it("opens the slash command menu for a / after text on the first line", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			editor.setAutocompleteProvider(
			...
			// Mid-line "fix spelling /" still opens the command menu.
			for (const ch of "fix spelling ") editor.handleInput(ch);
			editor.handleInput("/");
			await flushAutocomplete();
			...
			const rendered = editor
				...
				.map((l) => stripVTControlCharacters(l))
				.join("\n");
			assert.ok(rendered.includes("model"), "command menu should list model");
			assert.ok(rendered.includes("help"), "command menu should list help");

			// Typing letters keeps the menu filtered.
			editor.handleInput("m");
			await flushAutocomplete();
			assert.strictEqual(editor.isShowingAutocomplete(), true);
		});

		it("does not open the slash menu for a / inside a word", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			editor.setAutocompleteProvider(
				new CombinedAutocompleteProvider([{ name: "model", description: "Change model" }], process.cwd(), null),
			);

			// "packages/" is a path-like token, not a slash command; no menu opens.
			for (const ch of "packages/") editor.handleInput(ch);
			await flushAutocomplete();
			assert.strictEqual(editor.isShowingAutocomplete(), false);
		});

		it("applies a mid-line slash command without submitting the message", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			editor.setAutocompleteProvider(
				new CombinedAutocompleteProvider(
					[
			...
			for (const ch of "please run /mo") editor.handleInput(ch);
			await flushAutocomplete();
			assert.strictEqual(editor.isShowingAutocomplete(), true);

			// Enter applies the completion and keeps editing; does not submit.
			editor.handleInput("\r");
			await flushAutocomplete();
			assert.strictEqual(editor.getText(), "please run /model ");
			assert.strictEqual(submitted, undefined, "mid-line completion must not submit");
		});

		it("keeps a line-start slash command submitting on Enter", async () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			editor.setAutocompleteProvider(
				new CombinedAutocompleteProvider([{ name: "model", description: "Change model" }], process.cwd(), null),
			);
			...
			editor.handleInput("\r");
			assert.strictEqual(editor.getText(), "", "submit clears the editor");
			assert.strictEqual(submitted, "/model", "line-start completion submits the command");
		});
