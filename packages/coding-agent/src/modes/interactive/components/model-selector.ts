...
export class ModelSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		if (this.onSelectAsDefaultCallback) {
			this.addChild(
				new Text(
					theme.fg("dim", "  Enter to select and remember \u00b7 Ctrl+S to set as default \u00b7 Esc to cancel"),
					0,
					0,
				),
			);
	...
	private getMaxVisible(): number {
		...
		return typeof rows === "number" && Number.isFinite(rows) ? Math.max(1, Math.min(10, rows - 18)) : 10;
	}
	...
	private updateList(): void {
		...
		if (this.errorMessage) {
			...
			this.listContainer.addChild(
				new TruncatedText(theme.fg("muted", `  Model Name: ${selected.model.name}`), 0, 0),
			);
		}
