...
describe("TUI.queryTerminalBackgroundColor", () => {
	...
	it("coalesces repeated timed-out OSC 11 queries", async () => {
		const terminal = new TestTerminal();
		const tui: TUI = new TuiMainScreen(terminal);
		tui.start();
		try {
			const queries = Array.from({ length: 100 }, () => tui.queryTerminalBackgroundColor({ timeoutMs: 1 }));
			await wait(10);
			assert.deepStrictEqual(await Promise.all(queries), new Array(100).fill(undefined));
			const slots = Reflect.get(tui, "pendingOsc11BackgroundQueries") as Array<{ count?: number }>;
			assert.strictEqual(slots.length, 1);
			assert.strictEqual(slots[0]?.count, 100);
		} finally {
			tui.stop();
		}
	});
