...
export class FooterComponent implements Component {
	...
	private getUsageSnapshot(): FooterUsageSnapshot {
		const sessionManager = this.session.sessionManager;
		const entryCount = sessionManager.getEntryCount();
		const leafId = sessionManager.getLeafId();
		if (this.usageSnapshot && this.usageSnapshot.entryCount === entryCount && this.usageSnapshot.leafId === leafId) {
	...
	render(width: number): string[] {
		...
		const { usageTotals, latestCacheHitRate, contextUsage, sessionName } = this.getUsageSnapshot();

		// Context usage handles compaction; immediately after compaction, tokens are unknown until the next response.
		const contextWindow = contextUsage?.contextWindow ?? state.model?.contextWindow ?? 0;
		const contextPercentValue = contextUsage?.percent ?? 0;
		const displayedContextPercent = Math.min(100, Math.max(0, contextPercentValue));
		const contextPercent = typeof contextUsage?.percent === "number" ? displayedContextPercent.toFixed(1) : "?";
