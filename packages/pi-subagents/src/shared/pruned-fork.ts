...
export async function createPrunedForkSessionWriter(
	...
	// An explicit `forkContext.mode: "full"` opts out of pruning entirely.
	if (config?.mode === "full") return async () => {};

	// Determine the summarizer model. Explicit `pruned` config fails loudly on a
	// bad/missing model; the automatic mode degrades silently to a full fork.
	...
	if (config?.mode === "pruned") {
		...
		// Auto-prune large forked contexts by default to cut subagent token cost.
		// Use the parent's current model as a cheap summarizer; pruneForkSessionFile
		// already no-ops for small sessions (<= MAX_INHERITED_SESSION_BYTES), so only
		// large forks are summarized. Any failure falls back to the full fork so a
		// cost optimization can never break a subagent.
		const parent = ctx.model;
		if (parent?.provider && parent.id) modelId = `${parent.provider}/${parent.id}`;
	}
	if (!modelId) return async () => {};

	const available = ctx.modelRegistry.getAvailable();
	const resolved = resolveModelCandidate(modelId, available.map(toModelInfo), ctx.model?.provider);
	if (!resolved) {
		if (strict) throw new Error(`Pruned fork model '${modelId}' did not match exactly one available model.`);
		return async () => {};
	}
	const { baseModel } = splitKnownThinkingSuffix(resolved);
	const named = splitProviderModel(baseModel);
	if (!named) {
		if (strict) throw new Error(`Pruned fork model '${modelId}' must resolve to provider/model.`);
		return async () => {};
	}
	const model = ctx.modelRegistry.find(named.provider, named.id);
	if (!model) {
		if (strict) throw new Error(`Pruned fork model '${modelId}' was not found as '${baseModel}'.`);
		return async () => {};
	}
	const auth = await ctx.modelRegistry.getApiKeyAndHeaders(model);
	if (auth.ok === false) {
		if (strict) throw new Error(`Pruned fork model auth failed for ${baseModel}: ${auth.error}`);
		return async () => {};
	}
	...
	return async (sessionFile) => {
		if (strict) {
			...
			// Never let a cost optimization break a subagent: on any error, leave the
			// fork intact (full context) and let the child proceed normally.
			try {
				await pruneForkSessionFile(sessionFile, summarize);
			} catch {
				// fall back to the full fork
			}
		}
	};
}
