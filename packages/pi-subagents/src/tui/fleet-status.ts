		...
		return JSON.stringify({
			...
			entries: this.entries.map((entry) => this.active
				? [
					...
					// Round tokens to ~1K buckets so an actively streaming agent does not
					// force a repaint on every small token delta; the status line updates
					// ~once per 1K tokens instead of every tick.
					Math.round((entry.tokens ?? 0) / 1000),
