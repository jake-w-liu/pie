...
export interface AgentSessionConfig {
	...
	/** Side-effect-free projection of the next outbound context for compaction accounting. */
	projectContextForCompaction?: (messages: AgentMessage[]) => AgentMessage[];
...
export class AgentSession {
	...
	private _stopAfterTurnForContextLimit = false;
	...
	private readonly _projectContextForCompaction: (messages: AgentMessage[]) => AgentMessage[];
	...
	constructor(config: AgentSessionConfig) {
		...
		this._projectContextForCompaction = config.projectContextForCompaction ?? ((messages) => messages);
		...
		this._installAgentContextLimitStop();
	...
	private _estimateProjectedContextTokens(messages: AgentMessage[]): number {
		let projectedMessages: AgentMessage[];
		try {
			projectedMessages = this._projectContextForCompaction(messages);
		} catch {
			// Projection must not block a request. Raw messages are the conservative fallback.
	...
	private _installAgentNextTurnRefresh(): void {
		...
		this.agent.prepareNextTurnWithContext = async (turn, signal) => {
			...
			if (
				...
				if (currentCompactionId !== undefined && currentCompactionId !== previousCompactionId) {
					refreshedTurn = {
						...turn,
