...
Edit directly or use `/settings` for common options. Pie remembers models selected through `/model` or model cycling and uses the last selection at the next startup. Ctrl+S in `/model` remains an explicit way to save the selected model. To save the startup thinking level, use `/thinking` and press Ctrl+S.
...
|---------|------|---------|-------------|
| `defaultProvider` | string | - | Startup provider (e.g., `"anthropic"`, `"openai"`; updated by interactive model selection, Ctrl+S in `/model`, or manual editing) |
| `defaultModel` | string | - | Startup model ID (updated by interactive model selection, Ctrl+S in `/model`, or manual editing) |
...
| `enableInstallTelemetry` | boolean | `true` | Send an anonymous install/update version ping after first install or changelog-detected updates. This does not control package update checks |
...
### Telemetry and package update checks

`enableInstallTelemetry` only controls the anonymous install/update ping to `https://pi.dev/api/report-install`. Pie does not check for or install application releases. Interactive package availability checks remain independent of this setting.

Use `--offline` or `PI_OFFLINE=1` to disable startup package checks, model catalog refreshes, and install/update telemetry.
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `compaction.enabled` | boolean | `true` | Enable auto-compaction at the earlier of 87% context usage or the response-token reserve boundary |
| `compaction.reserveTokens` | number | `16384` | Tokens reserved for the LLM response; a larger reserve can trigger before 87% |
