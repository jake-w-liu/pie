...
describe("package commands", () => {
	...
	it("documents package updates without advertising application self-update", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		...
		const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
		expect(stdout).toContain("Update installed packages or refresh model catalogs");
		expect(stdout).toContain("itself is source-managed");
	...
	it("rejects removed self-update options even when combined with --models", async () => {
	...
	it("updates installed packages by default without checking for application releases", async () => {
		const fetchMock = vi.fn();
		vi.stubGlobal("fetch", fetchMock);
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

		await expect(runPackageCommandDirectly(["update"])).resolves.toBeUndefined();
	...
	it("rejects removed self-update options without making network requests", async () => {
		const fetchMock = vi.fn();
		vi.stubGlobal("fetch", fetchMock);
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
