...
Update installed packages or refresh model catalogs. Pie itself is source-managed.
...
Examples:
  ${APP_NAME} update                Update installed packages
	...
	if (command === "update") {
		if (modelsFlag) {
			...
			if (extensionsFlag && source) {
				conflictingOptions =
					conflictingOptions ?? "a positional update target cannot be combined with --extensions";
			}
			updateTarget = { type: "extensions", ...(source ? { source } : {}) };
		}
	}
...
export async function handlePackageCommand(
	...
	try {
		switch (options.command) {
			...
			case "update": {
				const updateSource = options.updateTarget?.type === "extensions" ? options.updateTarget.source : undefined;
