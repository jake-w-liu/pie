...
The 87% boundary keeps large-context models from reaching nearly 100% before compaction. The reserve boundary remains authoritative when it triggers earlier. Pie evaluates every completed assistant/tool turn before the next provider request, combining provider-reported usage with a side-effect-free projection of Headroom-compressed tool results. Compressible results are measured at their projected request size; skipped or uncompressible results retain their full size. If compaction cannot bring the projected context below the trigger, Pie stops automatic continuation instead of sending an over-limit request. The footer caps its percentage display at 100%, while raw token counts remain available for compaction and session statistics. By default, `reserveTokens` is 16384 tokens (configurable in `~/.pi/agent/settings.json` or `<project-dir>/.pi/settings.json`).
