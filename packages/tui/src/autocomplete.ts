...
/**
 * Extract a slash command token bounded by whitespace at the end of `text`,
 * e.g. "/model" from "please run /model". Used for mid-line slash commands.
 *
 * Returns null when the token is missing, not bounded by whitespace (so a
 * slash inside a word such as "packages/tui" stays a path), or path-like
 * (contains a second separator such as "/usr/bin").
 */
export function findSlashCommandToken(text: string): string | null {
	const lastDelimiter = Math.max(text.lastIndexOf(" "), text.lastIndexOf("\t"));
...
export class CombinedAutocompleteProvider implements AutocompleteProvider {
	...
	async getSuggestions(
		...
		if (!options.force) {
			const slashContext = this.extractSlashCommandContext(textBeforeCursor);
			if (slashContext !== null) {
				const { token, atLineStart, hasTrailingWhitespace } = slashContext;
				// Mid-line command followed by whitespace is an unreachable argument
				// region (argument completion is line-start only): close the menu
				// instead of pretending the text is a path and listing the cwd.
				if (!atLineStart && hasTrailingWhitespace) return null;
				...
				if (spaceIndex === -1) {
					...
					if (filtered.length === 0) {
						// No command matches. Mid-line slashes fall through to path completion
						// ("hello /usr" still suggests files); line-start keeps the historical
						// behavior of closing the menu.
						if (!atLineStart) {
							// continue to path extraction below
						} else {
							return null;
						}
					} else {
					...
					const commandName = token.slice(1, spaceIndex);
					const argumentText = token.slice(spaceIndex + 1);
				...
				// Mid-line command with an argument region ("hello /export ") has no
				// argument completions: close the menu instead of falling through to
				// path extraction, which would list the working directory.
	...
	applyCompletion(
		...
		// Slash commands are at the start of the line or in a whitespace-bounded token and don't
		// contain path separators after the first / . File items carry their own leading slash.
