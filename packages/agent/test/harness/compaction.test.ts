...
describe("harness compaction", () => {
	...
	it("returns a compaction result with file details", async () => {
		...
		// When the whole history is retained (nothing to summarize), compaction must
		// not issue an LLM call against an empty conversation; usage stays zero.
		expect(preparation!.messagesToSummarize.length).toBe(0);
		expect(result.usage?.totalTokens ?? 0).toBe(0);
