...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("mid-line slash commands", () => {
		const commands = [
			{ name: "model", description: "Change model" },
			{ name: "help", description: "Show help", argumentHint: "topic" },
		];

		it("offers command completions for a whitespace-bounded / token", async () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			const result = await getSuggestions(provider, ["please run /mo"], 0, 14);

			assert.notEqual(result, null);
			assert.strictEqual(result?.prefix, "/mo");
			assert.ok(result?.items.some((item) => item.value === "model"));
		});

		it("applies a mid-line command name completion in place", () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			const applied = provider.applyCompletion(["please run /mo"], 0, 14, { value: "model", label: "model" }, "/mo");

			assert.strictEqual(applied.lines[0], "please run /model ");
			assert.strictEqual(applied.cursorCol, "please run /model ".length);
		});

		it("keeps a second separator as a path", async () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			// "/usr/" has a second separator: path completion, never command items.
			const result = await getSuggestions(provider, ["please run /usr/"], 0, 16);

			assert.notEqual(result, null);
			assert.strictEqual(result?.prefix, "/usr/");
			assert.ok(!result?.items.some((item) => item.value === "model"));
		});

		it("treats a slash inside a word as a path, not a command", async () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			const result = await getSuggestions(provider, ["see packages/tui"], 0, 17);

			// Not a command token; result is path completion or null, never "model".
			assert.ok(result === null || !result.items.some((item) => item.value === "model"));
		});

		it("closes the menu when a mid-line command is followed by whitespace", async () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			// Argument completion is line-start only; a mid-line "\/model " must not fall
			// through to path extraction and list the working directory.
			const line = "please run /model ";
			const result = await getSuggestions(provider, [line], 0, line.length);
			assert.strictEqual(result, null);
		});

		it("does not double the slash when applying a forced path at a token boundary", () => {
			const provider = new CombinedAutocompleteProvider(commands, "/tmp");
			// File items carry the leading slash in their value; command completion must
			// not prefix another one.
			const applied = provider.applyCompletion(
				["please run /tmp"],
				0,
				15,
				{ value: "/tmp/foo", label: "foo" },
				"/tmp",
			);
			assert.strictEqual(applied.lines[0], "please run /tmp/foo");
		});
	});
