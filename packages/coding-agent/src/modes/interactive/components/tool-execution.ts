...
export class ToolExecutionComponent extends Container {
	...
	private maybeConvertImagesForKitty(): void {
		...
		for (const [index, converted] of this.convertedImages) {
			...
			if (current?.data !== converted.sourceData || current.mimeType !== converted.sourceMimeType) {
				this.convertedImages.delete(index);
			}
		}
		for (const [index, pending] of this.pendingImageConversions) {
			const current = imageBlocks[index];
			if (current?.data !== pending.sourceData || current.mimeType !== pending.sourceMimeType) {
		...
		for (let i = 0; i < imageBlocks.length; i++) {
			...
			const pending = this.pendingImageConversions.get(i);
			if (pending?.sourceData === img.data && pending.sourceMimeType === img.mimeType) continue;
