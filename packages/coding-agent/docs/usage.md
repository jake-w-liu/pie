...
| `/model` | Switch models and remember the selection for the next startup; Ctrl+S also saves it explicitly |
| `/thinking` | Switch thinking level; Ctrl+S in the picker saves the startup default |
| `/scoped-models` | Enable/disable models for Ctrl+P cycling |
| `/settings` | Theme, message delivery, transport, and other preferences |
| [`/headroom`](headroom.md) | Control request-time tool-result compression and view savings estimates |
...
- **Escape** clears the input without aborting. When a completion menu is open, it closes the menu first.
- **Ctrl+Alt+C** aborts and restores queued messages to the editor.
...
Pie enables [Headroom](headroom.md) by default. It replaces large historical tool-result text with smaller request-time previews while retaining exact originals in the session and a bounded in-process retrieval store. Use `/headroom status`, `/headroom on`, `/headroom off`, or `/headroom stats`.
...
- Automatic compaction starts at the earlier of 87% context usage or the configured response-token reserve; `/compact` summarizes older messages manually.
