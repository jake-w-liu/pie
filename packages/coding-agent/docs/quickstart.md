...
Additional built-in read-only tools (`grep`, `find`, `ls`) are available through tool options. Pie also enables the `headroom_retrieve` infrastructure tool so large historical tool results can be compressed safely in provider requests; see [Headroom](headroom.md). Pi runs in your current working directory and can modify files there. Use git or another checkpointing workflow if you want easy rollback.
...
- [Headroom](headroom.md) - request-time tool-result compression and exact retrieval.
- [Verified Pie setup](pi-setup-verified.md) - shipped package defaults and verification steps.
