...
describe("InteractiveMode compaction events", () => {
	...
	test("shows a recovery message when context remains unsafe after compaction", async () => {
		const fakeThis = { isInitialized: true, footer: { invalidate: vi.fn() }, showError: vi.fn() };
		const handleEvent = Reflect.get(InteractiveMode.prototype, "handleEvent") as (
			this: typeof fakeThis,
			event: { type: "context_limit"; tokens: number; contextWindow: number },
		) => Promise<void>;

		await handleEvent.call(fakeThis, { type: "context_limit", tokens: 150000, contextWindow: 100000 });

		expect(fakeThis.showError).toHaveBeenCalledWith(
			"Context remains at 150k/100k after auto-compaction. Automatic continuation stopped; use /compact, /new, or a larger-context model.",
		);
	});
