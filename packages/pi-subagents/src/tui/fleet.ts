...
function orderFleetAsyncRuns(runs: AsyncRunSummary[], terminalLimit: number): AsyncRunSummary[] {
	...
	// Active runs are ordered by when they STARTED (stable), so a running agent
	// bumping its updatedAt each tick cannot reorder the roster and yank the
	// user's selection to a different row. Tie-break by id for determinism.
	const startedAt = (run: AsyncRunSummary) => run.startedAt ?? run.lastUpdate ?? run.endedAt ?? 0;
	const byNewestStart = (left: AsyncRunSummary, right: AsyncRunSummary) =>
		startedAt(right) - startedAt(left) || (left.id < right.id ? 1 : left.id > right.id ? -1 : 0);
	const byNewest = (left: AsyncRunSummary, right: AsyncRunSummary) => updatedAt(right) - updatedAt(left);
	const active = runs.filter((run) => run.state === "queued" || run.state === "running").sort(byNewestStart);
...
export function collectFleetSnapshot(
	...
	for (const control of [...state.foregroundControls.values()].sort(
		(left, right) =>
			(right.startedAt ?? right.updatedAt ?? 0) - (left.startedAt ?? left.updatedAt ?? 0) ||
...
function itemStats(item: FleetItem): string[] {
	...
	// Round running durations to whole seconds so the rendered stat is stable
	// across sub-second refresh ticks; otherwise a running agent repaints its row
	// (and the detail header) on every tick, which the user sees as flashing.
	const roundSec = (ms: number): number => Math.round(ms / 1000) * 1000;
	if (item.kind === "foreground-active") {
		...
		durationMs = roundSec(Math.max(0, Date.now() - live.startedAt));
		...
		durationMs = roundSec(Math.max(0, externalElapsedEnd(item.run) - item.run.startedAt));
	} else {
		...
		durationMs = item.step?.durationMs ?? roundSec(Math.max(0, endTime - item.run.startedAt));
	}
	return [
		model,
...
function roundedRunningDuration(item: FleetItem): number {
	if (item.kind === "foreground-active") {
		return Math.round(Math.max(0, Date.now() - (item.activeChild ?? item.control).startedAt) / 1000);
	}
	if (item.kind === "external") {
		return Math.round(Math.max(0, externalElapsedEnd(item.run) - item.run.startedAt) / 1000);
	}
	if (item.kind === "async" && (item.state === "queued" || item.state === "running" || item.state === "pending")) {
		const endTime = item.run.endedAt ?? item.run.lastUpdate ?? Date.now();
		return Math.round(Math.max(0, endTime - item.run.startedAt) / 1000);
	}
	return -1;
}

function normalizeTokenTotal(tokens: number | TokenUsage | undefined): number | undefined {
	return typeof tokens === "number" ? tokens : (tokens?.total as number | undefined);
}

function itemTokens(item: FleetItem): number | undefined {
	if (item.kind === "foreground-active") return (item.activeChild ?? item.control).tokens;
	if (item.kind === "foreground-recent") return item.child.tokens;
	if (item.kind === "external") return undefined;
	const stepTotal = normalizeTokenTotal(item.step?.tokens);
	const runTotal = item.index === undefined ? normalizeTokenTotal(item.run.totalTokens) : undefined;
	return stepTotal ?? runTotal;
}

function itemToolCount(item: FleetItem): number | undefined {
	if (item.kind === "foreground-active") return (item.activeChild ?? item.control).toolCount;
	if (item.kind === "foreground-recent") return item.child.toolCount;
	if (item.kind === "external") return undefined;
	return item.step?.toolCount ?? (item.index === undefined ? item.run.toolCount : undefined);
}
...
export class SubagentFleetComponent implements Component {
	...
	private scheduleRefresh(): void {
		...
		this.refreshTimer = setTimeout(() => {
			...
			try {
				// Re-collect the snapshot but do NOT call invalidate(): invalidate()
				// clears transcriptCache, which forces a full disk re-read and
				// re-render of the selected run's transcript on every tick even
				// when nothing changed. renderedTranscript() already re-reads the
				// transcript when its fingerprint (file size/mtime) changes, so the
				// cache stays correct while idle ticks stay cheap (cache hit, no
				// writes emitted by the differential renderer). invalidate() is still
				// invoked by the base TUI on resize / full invalidation.
				this.refresh();
				// Skip a render pass when nothing visible changed (durations are
				// rounded to whole seconds, so an idle running agent no longer
				// repaints on every tick). The transcript fingerprint is part of the
				// key so detail growth still triggers a render.
	...
	private renderSignature(): string {
		const selectedKey = this.snapshot.items[this.selected]?.key;
		const rows = this.snapshot.items.map((item) =>
			`${item.key}|${item.agent}|${item.state}|${item.kind}|${itemTokens(item) ?? ""}|${itemToolCount(item) ?? ""}|${roundedRunningDuration(item)}`,
		);
		let transcriptFp = "";
		const selected = this.snapshot.items[this.selected];
		if (selected) {
			const target = transcriptTarget(selected, this.state);
			if (target) transcriptFp = transcriptFingerprint(target.path);
		}
		return `${selectedKey}\n${transcriptFp}\n${rows.join("\n")}`;
	}
