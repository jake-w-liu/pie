...
type TimedOutOsc11Replies = { timedOut: true; count: number };
type PendingOsc11Slot = PendingOsc11BackgroundQuery | TimedOutOsc11Replies;
...
export abstract class TuiBase extends Container implements TUI {
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ??
			process.env.PIE_CODING_AGENT_DIR ??
	...
	private markOsc11BackgroundQueryTimedOut(query: PendingOsc11BackgroundQuery): void {
		...
		if (previous && "timedOut" in previous) {
			...
			if (next && "timedOut" in next) {
				previous.count += next.count;
				this.pendingOsc11BackgroundQueries.splice(index, 1);
			}
		} else if (next && "timedOut" in next) {
			next.count += 1;
			this.pendingOsc11BackgroundQueries.splice(index, 1);
		} else {
			this.pendingOsc11BackgroundQueries[index] = { timedOut: true, count: 1 };
		}
	}
	...
	queryTerminalBackgroundColor({ timeoutMs }: { timeoutMs: number }): Promise<RgbColor | undefined> {
		return new Promise((resolve) => {
			...
			query.timer = setTimeout(() => {
				...
				query.resolve?.(undefined);
				query.resolve = undefined;
				this.markOsc11BackgroundQueryTimedOut(query);
			}, timeoutMs);
