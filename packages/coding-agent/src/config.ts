...
const cliEntrypoint = basename(process.argv[1] ?? "").toLowerCase();
const isPieEntrypoint = cliEntrypoint === "pie" || cliEntrypoint === "pie.exe" || cliEntrypoint.startsWith("pie-cli.");
const distributionName = process.env.PIE_DISTRIBUTION === "1" || isPieEntrypoint ? "pie" : piConfigName;
export const PACKAGE_NAME: string = pkg.name || "@earendil-works/pi-coding-agent";
export const APP_NAME: string = distributionName || "pi";
export const APP_TITLE: string = distributionName ? APP_NAME : "π";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".pi";
export const DEFAULT_PACKAGE_SOURCES: readonly string[] =
	APP_NAME === "pie" && Array.isArray(pkg.piConfig?.defaultPackages)
		? pkg.piConfig.defaultPackages.filter(
