import { type AutocompleteProvider, type AutocompleteSuggestions, findSlashCommandToken } from "../autocomplete.ts";
...
export class Editor implements Component, Focusable {
	...
	handleInput(data: string): void {
		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.select.confirm")) {
				const selected = this.autocompleteList.getSelectedItem();
				if (selected && this.autocompleteProvider) {
					// A slash command at the start of the message is submitted after completion
					// (e.g. "/model" + Enter runs the command). A mid-line slash command is
					// just inserted so the user can keep editing.
	...
	private insertCharacter(char: string, skipUndoCoalescing?: boolean): void {
		...
		if (!this.autocompleteState) {
			...
			// Auto-trigger for "/" at the start of a line or after whitespace (slash commands)
			if (char === "/" && this.isSlashCommandTokenStart(textBeforeCursor)) {
	...
	// True when the cursor sits right after a "/" that begins a slash command token:
	// at the start of the message or bounded by whitespace on the first line.
	// A "/" inside a word ("packages/tui") or a path with a second separator
	// ("/usr/bin") stays a path and does not open the command menu.
	private isSlashCommandTokenStart(textBeforeCursor: string): boolean {
		if (!this.isSlashMenuAllowed()) return false;
		if (this.isAtStartOfMessage()) return true;
		return findSlashCommandToken(textBeforeCursor) !== null;
	}
	...
	private handleTabCompletion(): void {
		...
		} else if (findSlashCommandToken(beforeCursor) !== null) {
			// Mid-line slash command token (e.g. "please run /mod"): complete the command,
			// not a file path. Paths with a second separator, e.g. "/usr/bin", still Tab
			// to file completion.
			this.handleSlashCommandCompletion();
		} else {
