...
function loadLargeSessionEntries(): SessionEntry[] {
	// Generate a large session in memory instead of loading a fixture file.
	resetEntryCounter();
	const entries: SessionEntry[] = [];
	for (let i = 0; i < 60; i++) {
		entries.push(
			createMessageEntry(createUserMessage(`User message ${i}`)),
			createMessageEntry(createAssistantMessage(`Assistant reply ${i}`, createMockUsage(100 + i, 50 + i))),
		);
	}
	entries.push(createMessageEntry(createUserMessage("Final user message")));
	entries.push(createMessageEntry(createAssistantMessage("Final assistant reply", createMockUsage(5000, 100))));
	return entries;
}
...
describe("shouldCompact", () => {
	it("triggers at the 87% ratio boundary when it is earlier", () => {
		...
		expect(shouldCompact(ratioBoundary - 1, contextWindow, settings)).toBe(false);
		expect(shouldCompact(ratioBoundary, contextWindow, settings)).toBe(true);
	});

	it("uses the reserve boundary when it is earlier than 87%", () => {
		const settings: CompactionSettings = {
			enabled: true,
			reserveTokens: 20000,
			keepRecentTokens: 20000,
		...
		expect(shouldCompact(79999, 100000, settings)).toBe(false);
		expect(shouldCompact(80000, 100000, settings)).toBe(true);
	});

	it("returns false when disabled or the context window is invalid", () => {
		const settings: CompactionSettings = {
			enabled: false,
			reserveTokens: 10000,
			keepRecentTokens: 20000,
...
describe("findCutPoint", () => {
	...
	it("keeps a trailing tool result with its assistant tool call", () => {
		const toolCallId = "tool-1";
		const assistantWithTool: AssistantMessage = {
			...createAssistantMessage("", createMockUsage(80, 0)),
			content: [{ type: "toolCall", id: toolCallId, name: "read", arguments: { path: "large.txt" } }],
			stopReason: "toolUse",
		};
		...
		const entries: SessionEntry[] = [
			...
			createMessageEntry(assistantWithTool),
