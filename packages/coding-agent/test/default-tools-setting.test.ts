...
describe("defaultTools setting", () => {
	...
	it("uses the configured list as the initial built-in selection", async () => {
		...
		expect(
			session
				...
				.map((tool) => tool.name)
				.sort(),
		).toEqual(["bash", "edit", "find", "grep", "headroom_retrieve", "ls", "powershell", "read", "write"]);
		expect(session.getActiveToolNames()).toEqual(["grep", "find", "headroom_retrieve"]);
	...
	it("can select powershell instead of bash", async () => {
		...
		expect(session.getActiveToolNames()).toEqual(["read", "powershell", "edit", "write", "headroom_retrieve"]);
	...
	it("keeps extension and SDK custom tools enabled", async () => {
		...
		expect(session.getAllTools().map((tool) => tool.name)).toEqual(
			expect.arrayContaining(["read", "dynamic_tool", "headroom_retrieve", "sdk_tool", "static_tool"]),
		);
		session.dispose();
	});
	...
	it("preserves explicit tool option precedence", async () => {
		...
		expect(excludedSession.getActiveToolNames()).toEqual(["grep", "headroom_retrieve"]);
	...
	it("applies through service-based session creation", async () => {
		...
		expect(session.getActiveToolNames()).toEqual(["ls", "headroom_retrieve"]);
		session.dispose();
	});
});
