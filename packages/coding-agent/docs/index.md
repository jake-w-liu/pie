...
> **Pie fork note:** this repo ships the `pie` command, so run every `pi <...>` example below as `pie <...>`. Package names (`@earendil-works/pi-*`), `~/.pi` paths, and `PI_*` environment variables are unchanged and shared with upstream Pi, so the full Pi extension ecosystem stays available.
